...
module.exports = function arrayChanges(actual, expected, equal, similar) {
    ...
    for (i = 0 ; i < mutatedArray.length ; i += 1) {
        var diffItem = mutatedArray[i];
        if (diffItem.type === 'similar') {
            if (equal(diffItem.value, diffItem.expected, diffItem.actualIndex, diffItem.expectedIndex)) {
                diffItem.type = 'equal';
            } else if (!similar(diffItem.value, diffItem.expected, diffItem.actualIndex, diffItem.expectedIndex)) {
                mutatedArray.splice(i, 1, {
                    type: 'remove',
                    value: diffItem.value,
                    actualIndex: diffItem.actualIndex
                }, {
                    type: 'insert',
                    value: diffItem.expected,
                    expectedIndex: diffItem.expectedIndex
                });
                if (diffItem.last) {
                    mutatedArray[i + 1].last = true;
                }
                i += 1;
            }
        }
